...
from src.entity.artifact_entity import DataIngestionArtifact,DataValidationArtifact,DataTransformationArtifact,ModelTrainerArtifact,ClassificationMetricsArtifacts,ModelEvaluationArtifact,ModelPusherArtifact
...
class TrainingPipeline:
    ...
    def start_model_evaluationn(self,model_evaluation_config,model_trainer_artifact,data_transformation_artifact,data_ingestion_artifact):
        try:
            logging.info("enter into model evaluation from training pipeline")

            model_evalution=ModelEvaluation(model_evaluation_config=model_evaluation_config,model_trainer_artifact=model_trainer_artifact,data_transformation_artifact=data_transformation_artifact,data_ingestion_artifact=data_ingestion_artifact)
            model_evaluation_artifact=model_evalution.initiate_model_evaluation()
            logging.info("model evaluation work completely done")
    ...
    def start_model_pusher(self,model_evaluation_artifact,model_pusher_config):

        try:
            logging.info("Enterd into model pusher from training pipeline")
            ...
            logging.info("Model pusher work done completely")
    ...
    def run_trainingpipeline(self):
        try:
            ...
            model_evaluation_artifact=self.start_model_evaluationn(model_evaluation_config=self.model_evaluation_config,model_trainer_artifact=model_trainer_artifact,
                                                data_transformation_artifact=data_transformation_artifact,data_ingestion_artifact=data_ingestion_artifact)
            model_pusher_artifact=self.start_model_pusher(model_pusher_config=self.model_pusher_config,model_evaluation_artifact=model_evaluation_artifact)
        except Exception as e:
            raise MyException(e,sys)
